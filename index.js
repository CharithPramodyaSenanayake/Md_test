...
async function startPeaceMD() {
    const PeaceMD = PeaceMDmousConnect({
        logger: pino({ level: 'debug' }),
        printQRInTerminal: true,
        browser: ['Peace-MD','Safari','2.0.0'],
        auth: state
    })
    ...
    // anticall auto block
    PeaceMD.ws.on('CB:call', async (json) => {
    const callerId = json.content[0].attrs['call-creator']
    if (json.content[0].tag == 'offer') {
    let pa7rick = await PeaceMD.sendContact(callerId, global.owner)
    PeaceMD.sendMessage(callerId, { text: `Automatic Block System!\nDon't Call Bot!\nPlease Ask Or Contact The Owner To Unblock You!`}, { quoted : pa7rick })
    await sleep(8000)
    await PeaceMD.updateBlockStatus(callerId, "block")
    }
    })
    ...
    PeaceMD.ev.on('connection.update', async (update) => {
        ...
        console.log('✅PEACE MD Successfully Connected... ', update)
    })
...
setInterval(() => {
     ...
     * @returns
     */
    PeaceMD.PeaceMDbtn = async (jid , text = '' , footer = '', img, but = [], options = {}) =>{
    ...
    PeaceMD.sendDocbtn = (jid, buttons = [], document, footer, quoted = '', options = {}) => {
        let buttonMessage = {
            document,
            footer,
            buttons,
    ...
    PeaceMD.sendVideobtn = (jid, buttons = [], video, footer, quoted = '', options = {}) => {
        let buttonMessage = {
            video,
            footer,
            buttons,
    ...
    PeaceMD.sendImgbtn = (jid, buttons = [], image, footer, quoted = '', options = {}) => {
        let buttonMessage = {
            image,
            footer,
            buttons,
    ...
    PeaceMD.sendLocbtn = (jid, buttons = [], location, footer, quoted = '', options = {}) => {
        let buttonMessage = {
            location,
            footer,
            buttons,
    ...
    PeaceMD.sendImageAsSticker = async (jid, path, quoted, options = {}) => {
        ...
        let buffer
        if (options && ('PeaceMDmous-MD' || 'Hiruwa')) {
